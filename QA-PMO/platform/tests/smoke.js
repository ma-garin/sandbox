 ...
 * Playwright(Chromium)で実際に描画し、全7ツールの動作とナビ/検索を検証する。
 */
...
(async () => {
  ...
  ok('ホームに全14サービスカード', await page.locator('#view-home .h-card').count() === 14);
  ok('ナビに実機能バッジ7件', await page.locator('#sidenav .tool-tag').count() === 7);
