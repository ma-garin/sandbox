---
name: nfr-standards
description: YukiのAIDDプロジェクト向けの非機能要件（NFR）定義スキル。spec.mdにNFRを書く時、パフォーマンス・アクセシビリティ・オフライン対応・セキュリティ・デバイス対応の要件を決める際は必ずこのスキルを使うこと。「非機能要件は」「パフォーマンスは」「アクセシビリティ対応」「オフラインで使えるか」「スマホ対応」という依頼にも適用する。
---

# NFR標準（Yuki AIDDプロジェクト用）
...
| **Streamlitアプリ** | pmo_agent系・ローカルまたはサーバ実行 |

---

## NFR: PWA（my-accounting-app / nichijo系）
...
### デバイス対応
- **Galaxy Z Fold5 カバー画面**: 360×820px で主要機能が使えること
- **Galaxy Z Fold5 メイン画面**: 768×1812px でレイアウトが崩れないこと
...
## NFR: 単一HTMLツール（UX Reviewer / QA Lens系）
