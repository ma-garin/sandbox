# Langfuse セルフホスト（NEC Mate SFF / Win11）

Weave相当のトレース・観測・データセット・LLM-as-judge UI を自前運用する。MITライセンス・課金ゼロ。

## 前提
- NEC Mate SFF（常時稼働）にDocker Desktop（Win11）
...
## Tailscale経由でFold5からアクセス
- NEC MateはすでにTailscale導入済み（M1 Mac開発と同じ網）
- Fold5から `http://<NEC-Mate-tailscale-ip>:3000` でトレースUIを外出先から確認できる

## アプリ側の計装（pmo_agent / stock swing）
```python
