...
| 項目 | 内容 |
|---|---|
| 技術スタック | Django 5.1 + SQLite（将来 PostgreSQL） |
...
| ツール | 内容 |
|---|---|
| ドキュメント検証 | 曖昧語・冗長文・必須節欠落を検出＋品質スコア |
| トレーサビリティ | 要件↔テストのRTM・カバレッジ算出 |
| 計画策定 | ISO 29119-3準拠のテスト計画書を生成 |
| テスト設計 | 観点ベース設計（観点カバレッジ付）＋境界値/同値/ペアワイズ |
| 欠陥管理 | ISTQB severity・DB永続化・CSV出力 |
| ROI計算機 | 捕捉率85%を年間削減額・3年ROIに翻訳 |
