...
echo "[スキル]"
for s in single-html-tool sdd-ecc-workflow qa-review-standards personal-pwa streamlit-rag-app design-system nfr-standards agent-eval; do
  check "$s" "$CLAUDE_DIR/skills/$s/SKILL.md"
done

echo "[コマンド]"
for c in new-pwa qa-review sdd-start token-check doc-search eval retro; do
